    ...
    <div>
      <section className="text-gray-400 bg-gray-900 body-font">
        <div className="container px-5 py-24 mx-auto">
          {Object.keys(products) == 0 && <p className='text-white text-xl text-center my-36'>Sorry all the mugs are currently out of stock. New stock comming soon!</p>}
  ...
  let mugs = {}
    for (let item of products) {
        if (item.title in mugs) {
            if (!mugs[item.title].color.includes(item.color) && item.availavleQty > 0) {
                mugs[item.title].color.push(item.color)
            }
            if (!mugs[item.title].size.includes(item.size) && item.availavleQty > 0) {
